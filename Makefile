...
PROJ_NAME = ted
LIBS = -lm
TEST_SUPPORT_SRC = src/src/modules/unity/unity.c
# Tenta find primeiro, se falhar usa wildcard
SRC_FILES := $(shell find src -name "*.c" ! -name "t_*.c" ! -path "$(TEST_SUPPORT_SRC)" 2>/dev/null)
ifeq ($(SRC_FILES),)
...
# Test files
TEST_FILES := $(shell find src -name "t_*.c" 2>/dev/null)
